...
export const fetchUsers = createAsyncThunk("users/fetchUsers", async () => {
    const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/users`, {
        headers: {
            Authorization: `Bearer ${localStorage.getItem("userToken")}`, // Admin token or similar authorization
        },
    });
    return response.data; // Assuming your API response contains the users' data
}
);
...
export const updateUser = createAsyncThunk("admin/updateUser", async ({ id, name, email, role }) => {
    await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/admin/users/${id}`,
        { name, email, role },
        {
            headers: {
                Authorization: `Bearer ${localStorage.getItem("userToken")}`,
